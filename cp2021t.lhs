...
sd_gen = either g1 (either g2 (either g3 g4))
        where g1 ()                       = (X, N 1.0)
              g2 a                        = (N a, N 0.0)
              g3 (Sum, ((a,b),(c,d)))     = (Bin Sum a c, Bin Sum b d)
              g3 (Product, ((a,b),(c,d))) = (Bin Product a c, Bin Sum (Bin Product a d) (Bin Product b c))
              g4 (E, (a,b))               = (Un E a, Bin Product (Un E a) b)
...
Analiticamente, podemos concluir que 

\begin{eqnarray}
	C_n_-_1 = \frac{2(n-1)!} {n! (n-1)! }
	\label{eq:cat}
\end{eqnarray} \vspace{0.2cm}
...
\begin{eqnarray}
	\frac {C_n} {C_n_-_1} = \frac{(2n)(2n-1)(2n-2)!(n-1)!} {(n+1)n(n-1)!(2n-2)!} = \frac {2(2n-1)} {n+1}
	\label{eq:cat}
\end{eqnarray} 

\begin{eqnarray}
	C_n = \frac{2(2n-1)} {n+1} C_n_-_1 = \frac {(4n-2)*C_n_-_1} {n+1}
	\label{eq:cat}
\end{eqnarray} \vspace{0.5cm}

Estabelecendo então a relação de recursividade pretendida.
